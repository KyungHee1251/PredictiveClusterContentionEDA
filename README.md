...
#### Executive summary
This project builds and evaluates machine learning models to predict workload failures and severe resource contention in large-scale cluster environments. Using Google Borg telemetry data as the training baseline, a Decision Tree model achieved near-perfect in-domain performance (0.999 accuracy and recall). To test cross-environment robustness, the model was validated on the Alibaba cluster trace dataset. While domain shift introduced false positives, the model successfully maintained a 1.000 recall score, proving its viability as a conservative, safety-first alerting tool to prevent unhandled cluster outages.
...
#### Results
* **In-Domain Performance:** The Decision Tree model excelled on Google Borg data, achieving 0.999 accuracy and 0.998 recall, driven primarily by features such as page_cache_memory, start_time, and memory_accesses_per_instruction.
...
* **Operational Trade-offs:** While recall was flawless, the cross-application validation revealed high false positives (8e+04), highlighting the impact of domain shift and the need for localized threshold calibration to mitigate alert fatigue.
...
- [Link to notebook: Data Exploration and Preprocessing](notebooks/01_data_prep.ipynb)


##### Contact and Further Information
For questions or collaboration inquiries regarding distributed systems performance engineering and predictive observability, please reach out via professional channels.
